...
public class Application_C1 {
	...
	Application_C1() {
		...
		Article fahrradhelm = new Article( "Fahrradhelm",  16900 ).setId( "SKU-663978" );
		...
		this.orders = new ArrayList<Order>( List.of( o8592, o3563, o5234, o6135,o7356, o4450, o6173 ) );
	}
	...
	void printOrder( OrderTableFormatter otfmt, Order order ) {
		...
		long amountCustomer = orders.subList(0, orders.indexOf(order)+1)
				.stream().filter(c->order.getCustomer().equals(c.getCustomer())).count();
		
		if(amountCustomer == 1) {
			customerOrder = order.getCustomer().getFirstName() + "'s order";
		} else if(amountCustomer == 2) {
			customerOrder = order.getCustomer().getFirstName() + "'s "+amountCustomer+"nd order";
		} else if(amountCustomer == 3) {
			customerOrder = order.getCustomer().getFirstName() + "'s "+amountCustomer+"rd order";
		} else {
			customerOrder = order.getCustomer().getFirstName() + "'s "+amountCustomer+"th order";
		}
		...
		for(int i = 0; i < items.length; i++) {
			StringBuffer priceForItem = fmtPrice(items[i].getArticle().getUnitPrice(), items[i].getArticle()
					.getCurrency());
			sumTotal = items[i].getArticle().getUnitPrice() * items[i].getUnitsOrdered();
			double taxItem = calculateIncludedVAT(items[i].getArticle().getUnitPrice(),items[i].getArticle()
					.getTax())/100.00;
	...
	long calculateIncludedVAT( Iterable<Order> orders ) {
		//long vat = 1318;		// hard-coded VAT of Eric's first order
		//
	...
	long calculateValue( Order order ) {
		//long value = 12979;		// hard-coded return value
		//
	...
	long calculateIncludedVAT( Order order ) {
		//
		//long vat = calculateIncludedVAT( 12979, TAX.GER_VAT );
		//
		...
		order.getItems().forEach(items::add);
		for(int i = 0; i < items.size(); i++) {
			double calculPrTax = calculateIncludedVAT(items.get(i).getArticle().getUnitPrice(), items.get(i).getArticle().getTax())/100.00;
	...
	private long calculateIncludedVAT( long price, TAX taxRate ) {
		//long vat = 1318;		// hard-coded return value
		//
